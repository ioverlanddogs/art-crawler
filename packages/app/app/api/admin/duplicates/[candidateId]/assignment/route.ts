...
export async function PATCH(request: Request, { params }: { params: { candidateId: string } }) {
  ...
  const current = await prisma.duplicateCandidate.findUnique({ where: { id: params.candidateId } });
  if (!current) return notFound('Duplicate candidate');
  ...
  if (action === 'claim' || action === 'assign' || action === 'reassign') {
    ...
    data = { assignedReviewerId: reviewerId, assignedAt: now, dueAt: current.dueAt ?? defaultDueAt(12, now), escalationLevel: 0, slaState: 'assigned' };
  } else if (action === 'escalate') {
    data = { escalationLevel: (current.escalationLevel ?? 0) + 1, slaState: 'escalated' };
  } else if (action === 'snooze') {
    data = { dueAt: snoozeDueAt(current.dueAt, payload?.snoozeHours ?? 2, now) };
  ...
  const updated = await prisma.duplicateCandidate.update({ where: { id: params.candidateId }, data: { ...data, slaState } });

  await prisma.pipelineTelemetry.create({ data: { stage: 'assignment', status: 'success', detail: `duplicate:${action}`, entityType: 'DuplicateCandidate', entityId: updated.id, metadata: { actorId: session.user.id, action, assignedReviewerId: updated.assignedReviewerId, escalationLevel: updated.escalationLevel } } });
